...
- `capture.remove`: drop the capture record only. Nodes and edges it created stay, and `history` keeps the original command.
- `node.put`: create or replace a node. `status: "archived"` retires a node without deleting it.
- `node.remove`: `ids[]` plus optional `removeEdges` and `rationale`. Refuses while incident edges exist — the error lists their ids — unless `removeEdges: true` cascades them away. Pending suggestions on removed endpoints become superseded. Captures keep their `nodeIds` as historical provenance.
